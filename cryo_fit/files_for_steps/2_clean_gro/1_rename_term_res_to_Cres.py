...
def run(args, log=sys.stdout):
    ...
    if len(args)<1: # both step 1-2 and 2-3 doesn't need argument, because each folder will have only 1 gro file
        ...
        cmd = "phenix.python 2_rename_term_res_to_Cres_by_resnum.py " + input_gro_file_name
        os.system(cmd)
        
        new_input_gro_file_name = input_gro_file_name[:-4] + "_c_term_renamed_by_resnum.gro"
        cmd = "phenix.python 3_rename_term_res_to_Cres_by_oc.py " + new_input_gro_file_name
        os.system(cmd)
    else:
        command_path = args[0]
        input_gro_file_name = args[1] # gro input file
        command_script = "phenix.python " + command_path + "steps/2_clean_gro/2_rename_term_res_to_Cres_by_resnum.py"
        print("command that will be executed: ", command_script)
        os.system(command_script)

        new_input_gro_file_name = input_gro_file_name[:-4] + "_c_term_renamed_by_resnum.gro"
        command_script = "phenix.python " + command_path + "steps/2_clean_gro/3_rename_term_res_to_Cres_by_oc.py "\
