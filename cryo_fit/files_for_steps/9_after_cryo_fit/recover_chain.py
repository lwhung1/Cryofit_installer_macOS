...
def retrieve_chain(seeking_res, seeking_res_num, search_after_this_line_num_in_ori_pdb,pdb_before_cryo_fit):
...
def run(args, log=sys.stdout):
   ...
   pdb_before_cryo_fit = args[0]
   pdb_after_cryo_fit = args[1]
   ##The above sys.argv values 2,0,1 was 3,1,2 in original commandline version
   ##Changes to be run as imported modules
   ...
   for line in pdb_after_cryo_fit_in:
      if line[:5] == "CRYST" or line[:3] == "END" or line[:6] == "HETATM" or \
         ...
         if ((res == former_res) and (res_num == former_res_num)):
            ...
            retrieved_chain, search_after_this_line_num_in_ori_pdb = retrieve_chain(res, res_num, search_after_this_line_num_in_ori_pdb,pdb_before_cryo_fit)
