...
def clean_main(input_pdb_file_name, bool_rna_name_reposition, bool_remove_MIA, bool_MIA_to_A, \
  ...
  # But gromacs made OC1, OC2 anyway, and it may have created cc=0.00 sometimes. So I turned it off
  ...
  #os.system(cmd)
  os.rename(output_pdb_file_name,final_output_pdb_file_name)
...
def clean_unusual_residue(input_pdb_file_name):
  ...
  cur_dir=os.getcwd()
  returned_file_dir=os.path.dirname(os.path.dirname(cur_dir))
  returned_file_path=os.path.join(returned_file_dir,'unusual_residue_removed.txt')
  returned_file_size = file_size(returned_file_path)
  
  if (returned_file_size == 0):
    #os.remove("../../unusual_residue_removed.txt")
    os.remove(returned_file_path)
  ...
  with open(output_pdb_file_name, "w") as f_out:
    with open(input_pdb_file_name) as f_in:
      ...
      for line in f_in:
        ...
        '''
        # trimmed L1 stalk can't identify the very 1st nucleic acid
        ...
        if trimmed_residue == "RA" or trimmed_residue == "RT" or trimmed_residue == "RU" \
          ...
          or trimmed_residue == "DA" or trimmed_residue == "DT" or trimmed_residue == "DU" \
          or trimmed_residue == "DG" or trimmed_residue == "DC" \
          or trimmed_residue == "A" or trimmed_residue == "T" or trimmed_residue == "U" \
          or trimmed_residue == "G" or trimmed_residue == "C" :
